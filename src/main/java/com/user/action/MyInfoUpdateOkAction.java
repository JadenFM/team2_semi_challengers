...
public class MyInfoUpdateOkAction implements Action {

	@Override
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response)
			throws IOException, Exception {
		// 마이페이지 - 회원정보 조회/수정 폼 페이지에서 넘어온 정보들로 회원 정보를 수정하는 비즈니스 로직.
		...
		String member_pwd2 = multi.getParameter("pwd2").trim();	// 필요 없는 값...
		...
		String memberImgFile = multi.getFilesystemName("mainImgFile");
		...
		// DB에 회원 정보를 수정하는 메소드 호출.
		int check = dao.updateMemberInfo(member_num, dto);
		...
		if(check > 0) {
			 ...
			 forward.setPath("member_mypage.do");
